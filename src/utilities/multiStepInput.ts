...
export async function multiStepInput(context: ExtensionContext) {
	...
	async function collectInputs() {
		...
		await MultiStepInput.run(input => inputTag(input, state));
		return state as State;
	}

	const title = 'Save current regex globally';
	...
	async function inputTag(input: MultiStepInput, state: Partial<State>) {
		...
		state.label = await input.showInputBox({
			...
			totalSteps: 2,
			value: state.label || '',
			prompt: 'Choose a unique name for this regex',
			validate: validateNameIsEmpty,
			shouldResume: shouldResume
		});
		return (input: MultiStepInput) => inputLang(input, state);
	}

	async function inputLang(input: MultiStepInput, state: Partial<State>) {
		state.lang = await input.showInputBox({
			title,
			step: 2,
			totalSteps: 2,
			value: state.lang || '',
			prompt: 'Choose a language for this regex, e.g. python, ruby',
			validate: validateNameIsEmpty,
			shouldResume: shouldResume
		});
	}
